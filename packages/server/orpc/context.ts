...
/** `request` works for both HTTP mount and SSR via createRouterClient - middleware reads cookies/auth headers from it. `db` is exposed so route-layer concerns (audit, ad-hoc queries) don't need a one-method service wrapper. */
